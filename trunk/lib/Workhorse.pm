...
my %fields = (
    config     => Workhorse::Config::object,
    connection => undef,
);
...
sub start_connection {
    ...
    openlog( 'workhorse', 'pid',
        $self->{config}->('global.daemon.client_syslog_facility') );
    ...
    $self->{connection}->reg_cb(
        ...
        contact_request_subscribe => sub {
            ...
            if (
                defined $users->{ $self->get_user( $contact->jid ) }
                ->{allowed} )
            {
        ...
        message => sub {
            ...
            if ( $msg->any_body ) {
                if (
                    defined $users->{ $self->get_user( $msg->from ) }
                    ->{allowed} )
                {
    ...
    for my $channel ( keys %{$channels} ) {
        ...
        $muc->join_room(
            ...
            sub {
                ...
                if ($err) {
                    syslog( 'error',
                        'Could not join ' . $channel . ': ' . $err->string );
                }
                else {
                    syslog( 'notice',
...
sub _handle_message {
    ...
    openlog( 'workhorse', 'pid',
        $self->{config}->('global.daemon.client_syslog_facility') );
    syslog( 'notice',
        'Direct message received from ' . $msg->from . ': ' . $msg->any_body );
